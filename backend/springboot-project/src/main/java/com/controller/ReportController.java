...
public class ReportController {
    private final ReportService reports; private final UserRepository users;
    public ReportController(ReportService reports, UserRepository users) { this.reports = reports; this.users = users; }
    ...
    @GetMapping("/reports") public List<Report> list(@RequestParam(required = false) ReportStatus status) { return reports.list(current(), status); }
