...
export default function CommunityDetail() {
  ...
  return <div>
    ...
    <TopBar eyebrow="Community workspace" title={community.name} subtitle={community.description || "Make shared decisions with your community."} action={community.joined ? <div className="detail-actions">{["OWNER", "MODERATOR"].includes(community.memberRole) && <Link className="btn ghost" to={`/reports?communityId=${community.id}`}>Moderation</Link>}<button className="btn brass" onClick={() => navigate(boardUrl)}>+ Create community board</button></div> : null} />
